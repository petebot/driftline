...
export function PeoplePage() {
  ...
  return (
    <div className="content-page people-page">
      <header className="page-bar page-bar--spacious"><div><p className="kicker">Operating network · Fictional people</p><h1>People</h1><p>Capacity, responsibility, and active work across the institute.</p></div><div className="people-rollup"><div><strong>{people.length}</strong><span>People</span></div><div><strong>{averageLoad}%</strong><span>Avg. allocated</span></div><div><strong>{openOwnership}</strong><span>Ownership gaps</span></div></div></header>

      <div className="listing-toolbar people-toolbar"><label className="search-field"><Search size={16} /><span className="sr-only">Search people</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search name, discipline, or location…" /></label><div className="stage-filters" aria-label="Filter people by role"><UsersRound size={14} /><button type="button" onClick={() => setRoleFilter("all")} className={roleFilter === "all" ? "is-active" : ""}>All <span>{people.length}</span></button>{roles.map((role) => <button type="button" key={role.id} onClick={() => setRoleFilter(role.id)} className={roleFilter === role.id ? "is-active" : ""}>{role.shortLabel} <span>{people.filter((person) => person.role === role.id).length}</span></button>)}</div></div>
        ...
        {results.map((person) => {
          ...
          return (
            <Link href={`/people/${person.id}`} className="person-card" key={person.id}>
              ...
              <div className="person-card__load"><div><span>Allocation</span><strong>{person.load}%</strong></div><span className="load-track"><i style={{ width: `${person.load}%` }} /></span><small>{person.load >= 80 ? "Limited room" : person.load <= 50 ? "Capacity available" : "Balanced load"}</small></div>
              <dl className="person-card__stats"><div><dt>Active work</dt><dd>{assignments.length}</dd></div><div><dt>Waiting</dt><dd>{personalAttention.length}</dd></div><div><dt>Stages</dt><dd>{stageCounts.length}</dd></div></dl>
