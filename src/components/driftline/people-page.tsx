        ...
        {results.map((person) => {
          ...
          return (
            <Link href={`/people/${person.id}`} className="person-card" key={person.id}>
              ...
              <div className="person-card__load"><div><span>Allocation</span><strong>{person.load}%</strong></div><span className="load-track" role="progressbar" aria-label={`${person.name} allocation`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={person.load}><i style={{ width: `${person.load}%` }} /></span><small>{person.load >= 80 ? "Limited room" : person.load <= 50 ? "Capacity available" : "Balanced load"}</small></div>
